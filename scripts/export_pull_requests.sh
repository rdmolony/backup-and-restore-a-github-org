...
for repo in "${REPOS[@]}"; do
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ "$(nix run nixpkgs#jq -- 'type' "${repo}_pull_requests.json")" = '"object"' ]; then
            actual_count=$(cat "${repo}_pull_requests.json" | nix run nixpkgs#jq -- '.data.repository.pullRequests.nodes | length')
        else
            actual_count=0
